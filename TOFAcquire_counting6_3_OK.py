...
from AcqirisPython import Acqiris
from AqMD3 import *
...
class AcquisitionThread(QtCore.QThread):
    ...
    def __init__(self, driver):
        ...
        self.Params = (1,0,driver.Acquisition.RecordSize,500) #Channel, ReadMode, Nsamp, TimeOut
        ...
        while self.Control == True:
            try:
                ...
                if self.AccOver == 0:
                    ...
                    self.Aqmd.Acquisition.Initiate()
                    timeoutInMs = self.Params[3]
                    self.Aqmd.Acquisition.WaitForAcquisitionComplete(timeoutInMs)
                    print("Acquisition completed")

                    waveform = None
                    for channel in self.Aqmd.Channels:
                        print()
                    ...
                    convertedSamples = np.array(convertedSamples)
                    self.Data = convertedSamples
                    
                    print("len(self.Data)", len(self.Data))
                    
                    # print(convertedSamples)
                    # print(type(convertedSamples))
                    # print(len(convertedSamples))
                    # print(type(convertedSamples[0]))
                ...
                # if self.AccOver == -1:
                #     print("if self.AccOver == -1:")
                #     print("self.Aqmd.Acquisition.RecordSize", self.Aqmd.Acquisition.RecordSize)
                #     self.Aqmd.Acquisition.Initiate()
                #     timeoutInMs = self.Params[3]
                #     print("timeoutInMs", timeoutInMs)
                #     self.Aqmd.Acquisition.WaitForAcquisitionComplete(timeoutInMs)
                #     print("Acquisition completed")
                ...
                #     self.Data_count=np.zeros(self.Params[2])
                #     step = (self.UI.DblIntTime.value() * 1e9 * 5) / self.Params[2]
                #     for i in range(0, self.Params[2], step):
                #         piece = Data[i:i+step]
                #         self.Data += piece
                        
                #     self.Data = np.array(self.Data)
                #     totalSteps = self.Params[2] / step
                #     self.Data = self.Data / totalSteps
                #     self.DataReady.emit()
                
                else: # AccOver>0
                    print("entered else")
                    print("driver.Acquisition.RecordSize", self.Aqmd.Acquisition.RecordSize)
                    ...
                    if self.Counter < self.AccOver:
                        ...
                        Data = waveform
                        if Acqrs.Status != 0:
                            print('Acqrs.Status = ', Acqrs.Status, ', ErrorSignal.emit() ')
                            self.Control = False
                            self.ErrorSignal.emit()
                        if self.Counter == 0:
                            ...
                            if self.timeAvg == 1:
                                ...
                                self.Data+=Data
                            print('self.Data type (else): ', type(self.Data))
                            #self.Data=np.array(self.Data)
                            print('self.Data type: ', type(self.Data))
                            #if self.ExtTrigg == True:
                        ...
                        #print("self.Data_count", self.Data_count)
                        if self.timeAvg == 1:
                            for wave in self.Waves:
                                step = self.IntSamples
                                self.accData = np.zeros(step)
                                for i in range(0, self.Params[2], step):
                                    piece = Data[i:i+step]
                                    self.accData += piece
                                
                                self.Data = np.array(self.accData)
                                totalSteps = self.Params[2] / step
                                self.Data = self.Data / totalSteps
                        else:
                            for sampleRaw in self.Data:
                                sampleRaw = sampleRaw * waveform.ScaleFactor
                            for sampleRaw in self.Data:
                                sampleRaw = sampleRaw + (len(self.Data) * waveform.ScaleOffset)
...
class MainUI(QtWidgets.QWidget):
    def __init__(self):
        ...
        Nsamp = self.UI.IntSamples.value()
        #Nsamp = self.UI.IntInterval.value() * 5
        self.numberSamples = Nsamp
        ...
        self.driver = AqMD3("PXI2::0::0::INSTR", True, True, "Simulate=false, DriverSetup= Model=U5310A")
        ...
        numPointsPerRecord = Nsamp
        self.driver.Acquisition.RecordSize = numPointsPerRecord
        ...
        self.AcqThread = AcquisitionThread(self.driver)
        
        self.AcqThread.AccOver = 0      
        self.AcqThread.AcqMode = self.UI.SlctMode.currentIndex()
    ...
    def RefreshPlot(self):          #Reitsma: for the covarinace mode the average data is displayed
        try:
            if self.UI.update_1.isChecked():
                ...
                #self.Time = np.arange(0, (self.numberSamples*2000), 2000)
                # self.Time = np.arange(0, (self.UI.IntInterval.value() * 5 *self.UI.IntInterval.value()), self.UI.IntInterval.value())
...
#        self.LogData = np.array([])
    ...
    def Configure(self):
        ...
        print("self.AcqThread.Params[2]", self.AcqThread.Params[2])
        self.AcqThread.Params = (1,0,self.driver.Acquisition.RecordSize,500)
        Intervals = np.array([1,2,5,10,20,50,100,200,500,1000,2000,5000])
        SampInt = self.UI.IntInterval.value()
        if SampInt in Intervals:
            pass
        ...
        print("self.UI.DblIntTime.value()", self.UI.DblIntTime.value())
        Nsamp = self.UI.IntSamples.value()
        #Nsamp = self.UI.IntInterval.value() * 5
        ...
        #self.Time = np.arange(0, (Nsamp *self.UI.IntInterval.value()), self.UI.IntInterval.value())
        print("self.UI.IntInterval.value()", self.UI.IntInterval.value())
        self.Time = np.arange(0, Nsamp / 5, 0.2)
        #self.Time = np.arange(0, Nsamp, 1)
        print("len(self.Time)", len(self.Time))
        ...
        coupling = VerticalCoupling.DC
        for channel in self.driver.Channels:
        # for channel in self.AcqThread.Aqmd.Channels:
            channel.Configure(range, offset, coupling, True)

        # Configure the acquisition.
        numPointsPerRecord = Nsamp
        self.driver.Acquisition.RecordSize = numPointsPerRecord
        # self.AcqThread.Aqmd.Acquisition.RecordSize = numPointsPerRecord
        ...
        slope = TriggerSlope.Positive
        self.driver.Trigger.ActiveSource = sourceName
        # self.AcqThread.Aqmd.Trigger.ActiveSource = sourceName
        activeTrigger = self.driver.Trigger.Sources[sourceName]
        # activeTrigger = self.AcqThread.Aqmd.Trigger.Sources[sourceName]
        activeTrigger.Level = level
        activeTrigger.Edge.Slope = slope
        activeTrigger.Coupling = coupling
        print(self.AcqThread.Aqmd)
        print("Driver initialized")
        ...
        if self.UI.SlctMode.currentIndex()==1:
            ...
            if self.UI.DblIntTime.value() == 0.0:
                ...
                self.AcqThread.timeAvg = 1
                self.AcqThread.IntSamples = self.UI.IntSamples.value()
                Waveforms = int(self.UI.DblIntTime.value() * 1000)
                print("Waveforms: ", Waveforms)
                print(self.AcqThread.Params)
                self.driver.Acquisition.RecordSize = 1000000000
                # self.AcqThread.Aqmd.Acquisition.RecordSize = 1000000000
                self.AcqThread.AccOver = int((self.UI.DblIntTime.value() * 1e9) / 200000000)
                # self.AcqThread.AccOver = -1
                # self.driver.Acquisition.RecordSize = int((self.UI.DblIntTime.value() * 1e9 * 5) / Nsamp)
                print("self.AcqThread.RecordSize: ", self.driver.Acquisition.RecordSize)
                self.AcqThread.Params = (Channel, 2, self.driver.Acquisition.RecordSize, Waveforms+500)
                DitherRange = 0
                TrigResync = 0
...
#            self.UI.BtnClearRef_2.setDisabled(True)
        ...
        if self.AcqThread.Control == False:
            print("self.AcqThread.Control: ", self.AcqThread.Control)
            self.AcqThread.wait()
            self.Configure()
