...
class AcquisitionThread(QtCore.QThread):
    ...
    def __init__(self):
        ...
        self.Params = (1,0,10000,500) #Channel, ReadMode, Nsamp, TimeOut
    ...
    def run(self):
        ...
        while self.Control == True:
            try:
                ...
                if self.AccOver == 0:
                    print("entered single acquisition")
                    #Acqrs.Acquire
                    print("acquire done")
                    
                    convertedSamples = Acqrs.Read()
...
#        self.LogData = np.array([])
    ...
    def Configure(self):
        ...
        self.AcqThread.Params = (1,0,self.UI.IntInterval.value()*5,500)
        ...
        self.Time = np.arange(0,self.UI.IntInterval.value() * 5 * SampInt,SampInt)
        ...
        if self.UI.SlctMode.currentIndex() in (0,2,3): #Reitsma: Added the 3rd mode to this list
            ...
            if self.UI.SlctTrigg.currentIndex()==0:
                print("if self.UI.SlctTrigg.currentIndex()==0:")
...
#            self.UI.BtnClearRef_2.setDisabled(True)
        ...
        #     #print('click start')
            ...
            Acqrs.driver.Calibration.SelfCalibrate()
            #Acqrs.Calibrate()
            #print("self.Calibrate done")
